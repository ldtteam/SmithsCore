            ...
            try {
                ...
                } else if (key.equalsIgnoreCase("f")) // Face Elements: f v1[/vt1][/vn1] ...
                {
                    if (splitData.length > 4)
                        SmithsCore.getLogger().log(Level.WARN, "OBJModel.Parser: found a face ('f') with more than 4 vertices, only the first 4 of these vertices will be rendered!");
                    ...
                    if (!unknownObjectCommands.contains(key)) {
                        ...
                        SmithsCore.getLogger().info("OBJLoader.Parser: command '%s' (model: '%s') is not currently supported, skipping. Line: %d '%s'", key, objFrom, lineNum, currentLine);
                    }
                }
