...
public class AsmUtils
{
    ...
    public static Field changeFieldAccess(Class<?> clazz, String fieldName, String srgName, boolean silenced)
    {
        try
        {
            Field f = clazz.getDeclaredField(SmithsCore.isIsInObfuscatedEnvironment() ? srgName : fieldName);
        ...
        catch (ReflectiveOperationException e)
        {
            if (!silenced)
            {
                SmithsCore.getLogger().error("Could not change access for field " + clazz.getSimpleName() + "."
                                               + (SmithsCore.isIsInObfuscatedEnvironment() ? srgName : fieldName), e);
            }
            return null;
        }
    }
    ...
    public static Method changeMethodAccess(Class<?> clazz, String methodName, String srgName, boolean silenced, Class<?>... params)
    {
        try
        {
            Method m = clazz.getDeclaredMethod(SmithsCore.isIsInObfuscatedEnvironment() ? srgName : methodName, params);
            m.setAccessible(true);
            return m;
        }
        catch (ReflectiveOperationException e)
        {
            SmithsCore.getLogger().error("Could not change access for method " + clazz.getSimpleName() + "."
                                           + (SmithsCore.isIsInObfuscatedEnvironment() ? srgName : methodName), e);
        }
